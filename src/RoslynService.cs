...
public partial class RoslynService
{
    ...
    private static readonly CSharpSyntaxStrategy _csSyntax = new();
    private static readonly VbNetSyntaxStrategy  _vbSyntax = new();

    private PEFileCache? _peFileCache;
    private IlDisassemblerAdapter? _ilDisassembler;

    private IlDisassemblerAdapter GetDisassembler()
    {
        _peFileCache ??= new PEFileCache();
        _ilDisassembler ??= new IlDisassemblerAdapter(_peFileCache);
        return _ilDisassembler;
    }

    private async Task<IAssemblySymbol?> FindAssemblySymbolAsync(string assemblyName)
    {
    ...
    private string? FindAssemblyPath(IAssemblySymbol assembly)
    {
    ...
    private static IEnumerable<INamedTypeSymbol> GetAllNamedTypesInNamespace(INamespaceSymbol ns)
    {
        ...
        foreach (var child in ns.GetNamespaceMembers())
            foreach (var type in GetAllNamedTypesInNamespace(child)) yield return type;
    }

    private ILanguageSyntaxStrategy SyntaxStrategy(Document doc)  => IsVisualBasicDocument(doc) ? _vbSyntax : _csSyntax;
    private ILanguageSyntaxStrategy SyntaxStrategy(bool isVb)     => isVb ? _vbSyntax : _csSyntax;
    private ICodeValidationStrategy ValidationStrategy(bool isVb) => isVb ? _vbSyntax : _csSyntax;
    ...
    /// Default is relative paths to save tokens. Set DOTNETLENS_ABSOLUTE_PATHS=true for absolute.
    ...
    public async Task<object> LoadSolutionAsync(string solutionPath)
    {
        ...
        (_solution, var strippedCount) = StripUnresolvedAnalyzerReferences(_solution);
        if (strippedCount > 0)
            Console.Error.WriteLine($"[Warning] Stripped {strippedCount} UnresolvedAnalyzerReference(s) — analyzer NuGet packages not restored. Run 'dotnet restore' if analyzer diagnostics are needed.");
        ...
        return CreateSuccessResponse(
            data: new
            {
                ...
                projectCount,
                documentCount,
                unresolvedAnalyzerReferencesStripped = strippedCount
            },
    ...
    /// MSBuildWorkspace creates UnresolvedAnalyzerReference objects for analyzer packages whose DLLs
    /// cannot be found. SymbolFinder.FindReferencesAsync/FindCallersAsync traverse all projects and
    /// hit an internal Roslyn switch that has no case for this type, throwing an exception.
    /// Stripping them is safe: unresolved references can never contribute analyzers anyway.
    private static (Solution solution, int strippedCount) StripUnresolvedAnalyzerReferences(Solution solution)
    {
        const string unresolvedTypeName = "Microsoft.CodeAnalysis.Diagnostics.UnresolvedAnalyzerReference";
        var strippedCount = 0;

        foreach (var project in solution.Projects)
        {
            var resolved = project.AnalyzerReferences
                .Where(r => r.GetType().FullName != unresolvedTypeName)
                .ToImmutableArray();

            var removedFromProject = project.AnalyzerReferences.Count - resolved.Length;
            if (removedFromProject > 0)
            {
                solution = solution.WithProjectAnalyzerReferences(project.Id, resolved);
                strippedCount += removedFromProject;
            }
        }

        return (solution, strippedCount);
    }
    ...
    {
        var serverVersion = typeof(RoslynService).Assembly
            .GetName().Version is { } ver ? $"{ver.Major}.{ver.Minor}.{ver.Build}" : "unknown";
    ...
    private void EnsureSolutionLoaded()
    {
        if (_solution == null)
        {
            throw new InvalidOperationException("No solution loaded. Call roslyn:load_solution first or set DOTNET_SOLUTION_PATH environment variable.");
        }
    }

    private void ClearWorkspaceCaches()
    {
        _documentCache.Clear();
        _compilationCache.Clear();
    }

    private static bool IsVbFilePath(string? filePath) =>
        filePath?.EndsWith(".vb", StringComparison.OrdinalIgnoreCase) ?? false;
    ...
    private async Task<bool> IsVisualBasicFileAsync(string filePath)
    {
        try
        {
            var document = await GetDocumentAsync(filePath);
            return IsVisualBasicDocument(document);
        }
        catch (FileNotFoundException) when (IsVbFilePath(filePath))
        {
            return true;
        }
    }

    private object VbNotSupportedResponse(string toolName) =>
        CreateErrorResponse(
            ErrorCodes.VbNotSupported,
            $"'{toolName}' does not support VB.NET files. " +
            "Semantic and symbol-level tools work across both C# and VB.NET. " +
            "Syntax-level operations (refactoring, code generation) are C# only.");

    private async Task<object?> GetCSharpOnlyToolErrorAsync(string? filePath, string toolName)
    {
        ...
        {
            var document = await GetDocumentAsync(filePath);
            return IsVisualBasicDocument(document) ? VbNotSupportedResponse(toolName) : null;
        }
        catch (FileNotFoundException) when (IsVbFilePath(filePath))
        {
            return VbNotSupportedResponse(toolName);
        }
    }
    ...
    internal async Task<Compilation?> GetProjectCompilationAsync(Project project)
    {
        ...
        try
        {
            ...
            GeneratorDriver? driver = project.Language switch
            {
                LanguageNames.CSharp when project.ParseOptions is CSharpParseOptions parseOptions =>
                ...
                LanguageNames.VisualBasic when project.ParseOptions is Microsoft.CodeAnalysis.VisualBasic.VisualBasicParseOptions parseOptions =>
                    Microsoft.CodeAnalysis.VisualBasic.VisualBasicGeneratorDriver.Create(
                        generators,
                        additionalTexts,
                        parseOptions,
                        project.AnalyzerOptions.AnalyzerConfigOptionsProvider),
    ...
    private sealed record DocumentSyntaxContext(
        Document Document,
        SyntaxTree SyntaxTree);

    private async Task<(DocumentSyntaxContext? Context, object? Error)> GetDocumentSyntaxContextAsync(
        string filePath,
        int line,
        int column)
    {
        ...
        var positionError = ValidateLineColumn(syntaxTree, filePath, line, column);
        if (positionError != null)
            return (null, positionError);

        return (new DocumentSyntaxContext(document, syntaxTree), null);
    }

    private async Task<(DocumentAnalysisContext? Context, object? Error)> GetDocumentAnalysisContextAsync(
        string filePath,
        int line,
        int column)
    {
        ...
        return (new DocumentAnalysisContext(document, semanticModel, syntaxTree), null);
    }

    private object? ValidateLineColumn(SyntaxTree syntaxTree, string filePath, int line, int column)
    {
        ...
        if (line < 0 || line >= text.Lines.Count)
        {
            return CreateErrorResponse(
                ...
                $"Line {line} is outside the document range.",
                hint: $"Use a line between 0 and {Math.Max(0, text.Lines.Count - 1)}.",
                context: new { filePath, line, column, lineCount = text.Lines.Count }
            );
        }
        ...
        if (column < 0)
        {
            return CreateErrorResponse(
                ErrorCodes.InvalidParameter,
                $"Column {column} is outside the document line range.",
                hint: $"Use a column of 0 or greater for line {line}.",
                context: new { filePath, line, column }
            );
        }
    ...
    private int GetPosition(SyntaxTree syntaxTree, int line, int column)
    {
        ...
        var normalizedColumn = Math.Min(column, sourceLine.End - sourceLine.Start);
        var linePosition = new Microsoft.CodeAnalysis.Text.LinePosition(line, normalizedColumn);
        return text.Lines.GetPosition(linePosition);
    }

    private (int StartPosition, int? EndPosition) GetPositionRange(
        SyntaxTree syntaxTree,
        int line,
        int column,
        int? endLine,
        int? endColumn)
    {
        ...
        var endPosition = endLine.HasValue && endColumn.HasValue
            ? GetPosition(syntaxTree, endLine.Value, endColumn.Value)
            : (int?)null;

        return (startPosition, endPosition);
    }

    private (ISymbol? Symbol, object? Error) ResolveSymbolAtPosition(
        DocumentAnalysisContext context,
        string filePath,
        int line,
        int column,
        string hint = "Try adjusting the line/column or use search_symbols to find symbols by name")
    {
        ...
        var symbol = symbolInfo.Symbol ?? context.SemanticModel.GetDeclaredSymbol(node);
        ...
        if (symbol == null)
        {
            return (null, CreateErrorResponse(
                ...
                "No symbol found at position",
                hint: hint,
                context: new { filePath, line, column, tokenText = token.Text, nodeKind = node.RawKind.ToString() }
            ));
        }
        ...
        int column)
    {
        ...
        var debugInfo = new Dictionary<string, object>
        {
            ...
            ["tokenKind"] = token.RawKind.ToString(),
        ...
        if (node != null)
        {
            debugInfo["nodeKind"] = node.RawKind.ToString();
        ...
        while (currentNode != null && walkCount < 5)
        {
            ...
            if (declaredSymbol != null)
            {
                ...
                debugInfo["foundNodeKind"] = currentNode.RawKind.ToString();
