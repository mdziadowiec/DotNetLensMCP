...
public class DiscoveryToolTests : RoslynServiceTestBase
{
    ...
    {
        var result = await Service.FindAttributeUsagesAsync("JsonConverter", projectName: "DotNetLensMcp");
        AssertSuccess(result);
        var data = GetData(result);
        var usages = data["usages"] as JArray;
        usages.Should().NotBeNull();
    ...
    {
        var result = await Service.GetDiRegistrationsAsync(projectName: "DotNetLensMcp");
        AssertSuccess(result);
    }
    ...
    {
        var result = await Service.FindReflectionUsageAsync(projectName: "DotNetLensMcp");
        AssertSuccess(result);
    }
    ...
    public async Task GetNuGetDependencies_ReturnsPackages()
    {
        ...
        var mainProject = projects!.FirstOrDefault(p => p["projectName"]?.Value<string>() == "DotNetLensMcp");
    ...
    {
        var result = await Service.GetNuGetDependenciesAsync(projectName: "DotNetLensMcp");
        AssertSuccess(result);
        var data = GetData(result);
        var projects = data["projects"] as JArray;
        projects!.Count.Should().Be(1);
        projects![0]["projectName"]?.Value<string>().Should().Be("DotNetLensMcp");
    }
    ...
    {
        var result = await Service.GetSourceGeneratorsAsync(projectName: "DotNetLensMcp");
        AssertSuccess(result);
    }
    ...
    {
        var result = await Service.GetGeneratedCodeAsync("DotNetLensMcp", "NonExistentFile.g.cs");
